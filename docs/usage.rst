...
From the previous code snippet: we also raise a :class:`.Error` in case no modifications are made to the content of the item.
...
when explicitly raising a :class:`.Error` or a :class:`.CriticalError` exception.
    ...
    class TextExtractor(Stage):
        ...
        def process(self, item: FilePathItem) -> DataItem:
            try:
                ...
                raise Error(f"Problems in reading file {item.path}") from e
            return item
