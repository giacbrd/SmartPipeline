...
def test_huge_run():
    ...
    pipeline = _pipeline()
    pipeline.set_source(FakeSource(200))
    pipeline.append_stage('reverser0', TextReverser(15000), concurrency=1, use_threads=False)
    pipeline.append_stage('reverser1', TextReverser(15000), concurrency=2, use_threads=False)
    pipeline.append_stage('reverser2', TextReverser(15000), concurrency=1, use_threads=False)
    pipeline.append_stage('duplicator', TextDuplicator(15000), concurrency=1, use_threads=False)
    runner = pipeline.run()
    start_time = time.time()
    items = list(runner)
